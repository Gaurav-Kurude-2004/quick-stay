...
const RoomDetails = () => {
    ...
    //check id the room is available
    const checkAvailability = async ()=>{
        try {
            //check is checkin date is greter than check out date
  ...
  return room && (
    ...
    <form onSubmit={onsubmithandler} className='flex flex-col md:flex-row items-start md:items-center justify-between bg-white shadow-[0px_0px_20px_rgba(0,0,0,0.15)] p-6 rounded-xl mx-auto mt-16 max-w-6xl'>
        ...
        <button type='submit' className='bg-primary hover:bg-primary-dull active:scale-95 transition-all text-white rounded-md max-md:w-full max-md:nt-6 md:px-25 py-3 md:py-4 text-base cursor-pointer'>
            {isAvailable ?"Book Now" :"Check Availiablity"}
        </button>
    </form>

    {/* common Specofications */}
...
{/* Hosted by */}
    <div className='flex flex-col items-start gap-4'>
        ...
        <button className='px-6 py-2.5 mt-4 rounded text-white bg-primary hoover:bg-primary-dull transition-all cursor-pointer'>Contact Now</button>
    </div>
